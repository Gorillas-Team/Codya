...
module.exports = class extends MusicCommand {
  ...
  run ({ channel, args, guild }) {
    ...
    if (isNaN(volume)) {
      return channel.sendTempMessage(this.client.botEmojis.error + ' | Você não informou o volume ou valor inserido é inválido.')
    }

    if (volume > 200) {
      return channel.sendTempMessage(this.client.botEmojis.error + ' | O volume inserido é superior a `200`.')
    }
