  ...
  async run({ channel, guild, member }) {
    ...
    if (guildDocument.djRole && member.roles.cache.has(guildDocument.djRole)) {
      ...
      return channel.sendTempMessage(
        this.client.getEmoji('skipped') + ' | Música pulada com sucesso.'
      )
    }

    if (guild.music.queue[0].skipVotes.includes(member.id)) {
      return channel.sendTempMessage(
        this.client.getEmoji('error') + ' | Você já votou.'
      )
    }
    ...
    if (guild.music.queue[0].skipVotes.length >= required) {
      guild.music.stop()
      return channel.sendTempMessage(
        this.client.getEmoji('skipped') +
          ' | A votação venceu! A música foi pulada com sucesso.'
      )
    }
